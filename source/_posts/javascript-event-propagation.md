...
為什麼會有「傳遞順序」這一詞呢？假設你有一個`ul`元素，底下有很多`li`，代表不同的 item，當你點擊任何一個`li`的時候，其實你也點擊了`ul`，因為`ul`把整個所有的`li`都包住了。

假如我在兩個元素上面都加上了`eventListener`，哪一個會先執行？這時候呢，知道事件的執行順序就很重要。
...
而這邊的`e`裡面就蘊含了許多這次事件的相關參數，其中有一個叫做`eventPhase `，是一個數字。
...
一樣是用大家所熟悉的`addEventListener`，而這個函數其實有第三個參數，`true`代表把這個 listener 添加到捕獲階段，`false`或是沒有傳就代表把這個 listener 添加到冒泡階段。

# 實際演練

大概知道事件的傳遞機制之後，我們拿我們上面寫好的那一個簡單範例來示範一下，一樣先附上事件傳遞的流程圖（假設我們點擊的對象是`#list_item_link`）
...
這樣子，當點擊超連結的時候，就不會執行原本預設的：新開分頁，而是沒有任何事情發生，這就是`preventDefault`的作用。
...
意思就是說這個你一旦 call 了這個，在之後傳遞下去的事件裡面也會有相同效果。
...
我們在`#list`的捕獲事件裡面就先寫了`e.preventDefault()`，而根據文件上面所說的，這一個效果會在之後傳遞的事件裡面一直延續。

因此，等之後事件傳遞到`#list_item_link`的時候，一樣會有相同效果，所以你會發現點超連結一樣沒反應。
...
而且這樣的好處是當你新增或是刪除一個 li 的時候，不用去處理跟那個元素相關的 listener，因為你的 listener 是放在 ul 身上。這樣透過父節點來處理子節點的事件，就叫做事件代理。
...
只要這樣一段程式碼，就可以把頁面上所有的元素停用，點了都沒有反應，像是`<a>`點了不會跳出超連結，`<form>`按了`submit`也沒用。
...
利用事件傳遞機制的特性，在`window`上面使用捕獲，就能保證一定是第一個被執行的事件，你就可以在這個 function 裡面偵測頁面中每一個元素的點擊，可以傳回去做數據統計之類的。
